...
import Core.Dashboards.ChartColumnPartDef;
...
public class ChartColumnPartControl extends ChartPartControl {
	...
	static class CustomBarRenderer1 extends BarRenderer {
		/**
		 * 
		 */
		private static final long serialVersionUID = 5736240414491134976L;
    ...
    private ChartColumnPartDef m_chartColumnPartDef;
	...
	public ChartColumnPartControl(ISiteviewApi iSiteviewApi,
	...
	protected void DefineFromDef(DashboardPartDef def, PartRefDef partRefDef)
    {
        ...
        if (super.get_DashboardPartDef()!= null && super.get_DashboardPartDef() instanceof ChartColumnPartDef){
	        ChartColumnPartDef dashboardPartDef = (ChartColumnPartDef) super.get_DashboardPartDef();
	        if (dashboardPartDef != null)
	        {
	            this.m_chartColumnPartDef = dashboardPartDef;
	...
	public  void DataBind(java.lang.Object dt){
        ...
        if(this.m_GridMode){
            ...
            if (table.get_Rows().get_Count() == 0){
            	//无资料时,加了此句出错
//                table = DataConverter.InsertEmptyRowToTable(DataConverter.ConvertColumnNameAndType(table, this.m_chartColumnPartDef.get_GroupByDef(), this.m_chartColumnPartDef.get_SubGroupByDef(), this.m_chartColumnPartDef.get_EvaluateByDef()), false);
            }
            else{
            	//
            }
            ...
            if(table.get_Rows().get_Count()>1){
	            ...
	            for(int i = 0; i < table.get_Rows().get_Count();i++){
	            	...
	            	for(int j = 1; j < table.get_Columns().get_Count();j++){
	            		...
	            		String s2 = dc.get_ColumnName();
	            		double v = getDoubleValue(dr.get_Item(dc));
	            		if(m_chartColumnPartDef.get_LegendVisible()){
	            			dataset.addValue(v, s2, s1);
	        			}else{
            ...
            m_ultraChart = ChartFactory.createBarChart("", // chart title
    				...
    				m_chartColumnPartDef.get_LegendVisible(),//false, // include legend
    				true, // tooltips?
    				false // URLs?
    				);
            ...
            m_ultraChart.setBackgroundImage(SwtImageConverter.ConvertToAwtImage(ImageResolver.get_Resolver().ResolveImage(m_chartColumnPartDef.get_BackImageName())));
            m_ultraChart.setBackgroundPaint(ConvertUtil.toAwtColor(m_chartColumnPartDef.get_BackColor()));
            if(m_chartColumnPartDef.get_LegendVisible()){
    		...
    		plot.setBackgroundPaint(ConvertUtil.toAwtColor(m_chartColumnPartDef.get_BackColor()));
    		...
    		if(dataset.getRowCount()>1){
    			...
    			renderer1 = new CustomBarRenderer1();
         		...
         		if(dataset.getColumnCount()<=1){
	         		...
	         		renderer1.setBaseItemLabelFont(new Font("simsun", Font.PLAIN, 12));
	         		renderer1.setItemLabelAnchorOffset(-10D);  

	         		//ItemLabelPosition localItemLabelPosition1 = new ItemLabelPosition(ItemLabelAnchor.INSIDE2, TextAnchor.CENTER_RIGHT, TextAnchor.CENTER_RIGHT, -1.570796326794897D);
	         		//renderer1.setBasePositiveItemLabelPosition(localItemLabelPosition1);
	         	   // ItemLabelPosition localItemLabelPosition2 = new ItemLabelPosition(ItemLabelAnchor.INSIDE2, TextAnchor.CENTER_RIGHT, TextAnchor.CENTER_RIGHT, -1.0);
	         	    //renderer1.setPositiveItemLabelPositionFallback(localItemLabelPosition2);
         		}
	...
	private void showPanel(Composite p) {
		...
		if(this.get_MainArea()!=null && !this.get_MainArea().isDisposed()){
...
//				}
	...
	private Paint[] createPaint() {
        ...
        colors[0] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor1())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        colors[1] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor2())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        colors[2] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor3())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        colors[3] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor4())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        colors[4] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor5())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        colors[5] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor6())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        colors[6] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor7())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        colors[7] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor8())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        colors[8] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor9())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        colors[9] = ConvertUtil.toAwtColor(ColorResolver.get_Resolver().ResolveColor(this.m_chartColumnPartDef.get_CustomColor10())); //Color.red;//new GradientPaint(0f, 0f, Color.red, 0f, 0f, Color.white);
        return colors;
    }
	...
	public Object GetData(ILoadingStatusSink sink){
        ...
        try{
            ...
            if ((!super.get_ChartPartDef().get_GroupByDef().get_Duration() && super.get_ChartPartDef().get_GroupByDef().get_LimitQuery()) && (!super.get_ChartPartDef().get_GroupByDef().get_OrderByFieldName().equals(super.get_ChartPartDef().get_GroupByDef().get_FieldName()) && !super.get_ChartPartDef().get_GroupByDef().get_OrderByFieldName().equals(super.get_ChartPartDef().get_BusObName() + "." + super.get_ChartPartDef().get_EvaluateByDef().get_FieldName()))){
                dt = DataConverter.RemoveOrderByColumn(dt, this.m_chartColumnPartDef.get_GroupByDef(), this.m_chartColumnPartDef.get_SubGroupByDef(), this.m_chartColumnPartDef.get_EvaluateByDef());
            }
            if (sink != null){
            ...
            dt = DataConverter.ConvertColumnNameAndType(dt, this.m_chartColumnPartDef.get_GroupByDef(), this.m_chartColumnPartDef.get_SubGroupByDef(), this.m_chartColumnPartDef.get_EvaluateByDef());
            if (sink != null){
                sink.ChangeProgress(30, 100);
            }
            dt = DataConverter.ConvertIntervalNumberToProperName(dt, this.m_chartColumnPartDef.get_GroupByDef(), this.m_chartColumnPartDef.get_SubGroupByDef());
            if (sink != null){
        ...
        if (m_chartPanel.getVisible()){
            ...
            if (vDateTimeDef != null){
                if (vDateTimeDef.get_IsCustom()){
                    query = DateRangeCriteriaCreator.AddCustomDateRangeCriteria(query, this.m_chartColumnPartDef.get_DefaultDateRangeDef().get_DateTimeField(), vDateTimeDef.get_StartDateTime(), vDateTimeDef.get_EndDateTime());
                }else if (vDateTimeDef.get_IsFiscalPeriod()){
                    query = DateRangeCriteriaCreator.AddFiscalDateRangeCriteria(super.getApi(), query, this.m_chartColumnPartDef.get_DefaultDateRangeDef().get_DateTimeField(), vDateTimeDef.get_FiscalPeriod());
                }else{
                    query = DateRangeCriteriaCreator.AddDateRangeCriteria(super.getApi(), query, vDateTimeDef.get_DateRange(), this.m_chartColumnPartDef.get_DefaultDateRangeDef().get_DateTimeField());
                }
            }
            this.gridControl.set_Query(query);
            this.DoDataRefresh();
