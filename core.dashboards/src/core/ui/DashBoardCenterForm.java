...
import core.ui.dialog.DashboradDialog;

public class DashBoardCenterForm extends ApplicationWindow{

	private static String m_strDefClassName="DashboardDef";  // C#查询数据时的固定参数
	private ScopeUtil m_ScopeUtil;     // 暂时叫  查询范围的类
	private static String m_moduleSecurityName="Core.Security.Dashboards";  // C#查询数据时的固定参数
	private PerspectiveDef perspectiveDef;  // 不懂
	private DashBoardCenterForm quickcenter;// 主程序窗口
	...
	private Combo cboQuickOblink;  //下拉式  
	...
	private boolean serviceorchclient;
	 ...
	 */
	public DashBoardCenterForm(boolean flag,String bus_name,boolean serviceorchclient) {
		super(null);
...
//		addStatusLine();
	...
	public DashBoardCenterForm(String bus_name, PerspectiveDef perspectiveDef, DefinitionLibrary m_Library, boolean showButtons,boolean serviceorchclient){
		super(null);
		this.setShellStyle(SWT.CLOSE | SWT.MAX | SWT.RESIZE| SWT.APPLICATION_MODAL);
							...
							if (owners.get_Scope() == ph.get_Scope()) {
								if (!StringUtils.IsEmpty(ph.get_Folder())) {
									if (!set.contains(ph.get_Folder())) {
										...
										node.setImage(SwtImageConverter.ConvertToSwtImage(ImageResolver.get_Resolver()
														.ResolveImage( "[IMAGE]Core#Images.Icons.FolderClosed.png", 0x12, 0x12)));
										node.setText(ph.get_Folder());
										node.setData(str);
									}
									set.add(ph.get_Folder()); 
	...
	public void addlisttabledate(String sel){
		...
		if(obs!=null&&obs.get_Count()>0){
			for(int i = 0 ; i < obs.get_Count(); i++){
				...
				ti.setImage(SwtImageConverter.ConvertToSwtImage(ImageResolver.get_Resolver().ResolveImage("[IMAGE]Core#Images.Icons.BusinessProcess.png"),0x12,0x12));
...
//	}
	...
	protected CoolBarManager createCoolBarManager(int style) {
		...
		if(!serviceorchclient)toolbar.add(qrun);
		toolbar.add(actAdd);
		toolbar.add(actEdit);
		toolbar.add(actCopy);
	...
	private void createActions(){
		...
		actAdd = new Action("新建(&N)"){
			...
			public void run() {
				...
				m_Def =m_Library.GetNewDefForEditing(DefRequest.ByCategory(scope_int, "", DashboardDef.get_ClassName(), ""));
				dashboardDef = (DashboardDef)m_Def;
				DashboradDialog dashbord=new DashboradDialog(quickcenter,getShell(), dashboardDef,m_api, m_Library, m_ScopeUtil,m_Def, cboQuickOblink.getData(cboQuickOblink.getItem(cboQuickOblink.getSelectionIndex())).toString(), 1);
		...
		actEdit = new Action("编辑(&E)"){
			...
			public void run() {
				...
				if ((defForEditing != null) && HasEditRight(defForEditing.get_Scope())){
					dashboardDef=(DashboardDef) defForEditing;
					
					Dialog dbPart = new DashboradDialog(quickcenter,getShell(), dashboardDef,m_api, m_Library, m_ScopeUtil,null, ph.get_LinkedTo(), 2);
					getShell().setCursor(getShell().getDisplay().getSystemCursor(SWT.CURSOR_WAIT));
					if(0 == dbPart.open())
					{
						if(serviceorchclient)
						{
							SynchronizeCoreProperties(dashboardDef);
							ReplaceDefinitionInformationInRunMru(dashboardDef);
						}
...
//				}
				...
				TableItem ti = tbBusObList.getSelection()[0];
				PlaceHolder ph = (PlaceHolder)ti.getData();
				if(ph != null && HasAddRight(ph.get_Scope()))
				{
					m_Def =m_Library.GetNewDefForEditing(DefRequest.ByCategory(ph.get_Scope(), "", DashboardDef.get_ClassName(), ""));
					dashboardDef = (DashboardDef)m_Library.GetDefForEditing(DefRequest.ByHolder(ph));
					DashboradDialog dashbord=new DashboradDialog(quickcenter,getShell(), dashboardDef,m_api, m_Library, m_ScopeUtil,m_Def, cboQuickOblink.getData(cboQuickOblink.getItem(cboQuickOblink.getSelectionIndex())).toString(),3);
					getShell().setCursor(getShell().getDisplay().getSystemCursor(SWT.CURSOR_WAIT));
					if(0 == dashbord.open())
					{
...
//                                   }
		...
		qrun = new Action("运行(&R)"){
			...
			public void run() {
				...
				if ((defForEditing != null) && HasEditRight(defForEditing.get_Scope())){
					dashboardDef=(DashboardDef) defForEditing;
						DashboardPanel.open(m_api,dashboardDef);
						getShell().close();
				}
			}
		};
		qrun.setImageDescriptor(new ImageDescriptor(){
			@Override
			public ImageData getImageData() {
				return SwtImageConverter.ConvertToSwtImage(ImageResolver.get_Resolver().ResolveImage("[Image]Siteview#Images.Icons.Common.RunItem.png",0x12,0x12)).getImageData();
			}});
...
//        }
	...
	public boolean HasAddRight(int s)
    {
        return ((((s != Scope.Unknown)) && (DashBoardCenterForm.m_moduleSecurityName != null)) && (DashBoardCenterForm.m_moduleSecurityName != "")) && this.m_api.get_SecurityService().HasModuleItemRight(m_moduleSecurityName, getScopeString(s), SecurityRight.Add);
    }
	 ...
	 public Object UnOverrideObject(Object o)
     {
         ...
         if (this.perspectiveDef != null)
         {
             ...
             if (def != null)
             {
                 ...
                 IDefinition definition2 = m_Library.GetDefinition(DefRequest.ById(def.get_Scope(), def.get_ScopeOwner(), "", "DashboardDef", def.get_Id(), "(Base)", false));
...
//             }
    ...
    {
        IDefinition defOriginal = this.m_Library.GetDefinition(DefRequest.ById(def.get_OriginalScope(), def.get_OriginalScopeOwner(), "", "DashboardDef", def.get_Id(), def.get_Perspective(), false));
        if (defOriginal != null)
        {
	...
	protected void ReplaceDefinitionInformationInRunMru(IDefinition def)
    {
        if (def != null)
        {
            ...
            if (mru != null)
            {
                String strMruId = "".equals(def.get_LinkedTo()) ? "DashboardDef.Run" : "DashboardDef.Run" + "." + def.get_LinkedTo();
